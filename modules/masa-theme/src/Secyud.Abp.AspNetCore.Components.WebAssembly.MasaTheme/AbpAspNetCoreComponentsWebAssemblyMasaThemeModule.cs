...
public class AbpAspNetCoreComponentsWebAssemblyMasaThemeModule : AbpModule
{
    ...
    {
        Configure<AbpRouterOptions>(options => { options.AdditionalAssemblies.Add(typeof(AbpAspNetCoreComponentsWebAssemblyMasaThemeModule).Assembly); });

        context.Services.AddAutoMapperObjectMapper<AbpAspNetCoreComponentsWebAssemblyMasaThemeModule>();

        Configure<AbpAutoMapperOptions>(options => { options.AddMaps<AbpAspNetCoreComponentsWebAssemblyMasaThemeModule>(); });
    }
}